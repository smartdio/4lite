...
const relative='public/assets/models/building-openings/building-opening-shared-textures-v01.glb'
const sourcePath=path.resolve(root,relative)
const apply=process.argv.includes('--apply')
const outputFlag=process.argv.indexOf('--output')
const outputPath=apply?sourcePath:path.resolve(root,outputFlag>=0?process.argv[outputFlag+1]:'artifacts/performance/phase6d/candidate/building-opening-shared-textures-v01.glb')
...
const archivePath=path.resolve(root,'archive/phase-6d-pre-webp',relative)
...
const temporaryDirectory=await mkdtemp(path.join(tmpdir(),'4lite-opening-webp-'))
try {
  for(const [imageIndex,image] of (json.images??[]).entries()) {
    ...
    const stem=`opening-${imageIndex}`
    const input=path.join(temporaryDirectory,`${stem}.png`),webpPath=path.join(temporaryDirectory,`${stem}.webp`)
    await writeFile(input,png)
    const isRoughness=image.name.includes('roughness')
    const isGlass=image.name.includes('old-glass')
    const args=isRoughness
      ?['-quiet','-lossless','-m','6','-exact',input,'-o',webpPath]
      :['-quiet','-q',isGlass?'95':'92','-m','6','-sharp_yuv','-alpha_q','100','-alpha_filter','best','-exact',input,'-o',webpPath]
    ...
    if(comparisonDirectory) {
      ...
      const safeName=image.name.replaceAll(/[^a-zA-Z0-9._-]/g,'-')
      const originalPath=path.join(comparisonDirectory,`${safeName}-original.png`)
      const decodedPath=path.join(comparisonDirectory,`${safeName}-decoded.png`)
      await copyFile(input,originalPath)
      await run('dwebp',['-quiet',webpPath,'-o',decodedPath])
      const {stdout:dimensions}=await run('magick',['identify','-format','%wx%h %[channels]',originalPath])
      comparison={dimensions:dimensions.trim(),normalizedRmse:await normalizedRmse(originalPath,decodedPath)}
      if(isGlass) {
        ...
        await run('magick',[originalPath,'-alpha','extract',originalAlpha])
        await run('magick',[decodedPath,'-alpha','extract',decodedAlpha])
        comparison.alphaNormalizedRmse=await normalizedRmse(originalAlpha,decodedAlpha)
      }
    }
    ...
    details.push({imageIndex,name:image.name,mode:isRoughness?'lossless':isGlass?'q95-alpha100-exact':'q92-alpha100-exact',beforeBytes:png.length,afterBytes:webp.length,savedBytes:png.length-webp.length,comparison})
  }
} finally {
  await rm(temporaryDirectory,{recursive:true,force:true})
}

if(details.length!==6)throw new Error(`Expected 6 PNG images, found ${details.length}`)
...
const report={
  ...
  policy:'opaque baseColor q92; transparent old-glass baseColor q95; alpha quality 100 exact; roughness lossless; dimensions unchanged',
  beforeBytes:source.length,afterBytes:output.length,savedBytes:source.length-output.length,savedRatio:1-output.length/source.length,
  images:details,
}
