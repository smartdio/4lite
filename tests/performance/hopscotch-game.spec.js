...
test('nine-cell fan boundaries report chalk lines instead of leaking into adjacent rows',async({page})=>{
  ...
  expect(state).toMatchObject({phase:'fault',faultReason:'瓦片压线了',reasonCode:'tile-line'})
...
test('hopscotch rejects short, over, wrong, line and invalid foot landings after motion',async({page})=>{
  ...
  expect(state).toMatchObject({phase:'fault',faultReason:'投短了',reasonCode:'throw-short'})
  await resetRound(page)
  state=await page.evaluate(()=>window.__CAMPUS_TEST__.throwHopscotchTile(1,0,5))
  expect(state).toMatchObject({phase:'fault',faultReason:'投过格了',reasonCode:'throw-long'})
  await resetRound(page)
  state=await page.evaluate(()=>window.__CAMPUS_TEST__.throwHopscotchTile(2,0,0))
  expect(state).toMatchObject({phase:'fault',faultReason:'投错格了',reasonCode:'wrong-tile'})
  await resetRound(page)
  ...
  expect(state).toMatchObject({phase:'fault',faultReason:'踩进瓦片格了',reasonCode:'stepped-in-tile'})
  await resetRound(page);await page.evaluate(()=>window.__CAMPUS_TEST__.throwHopscotchTile(1,0,0))
  state=await hop(page,2,.3,'single')
  expect(state).toMatchObject({phase:'fault',faultReason:'踩线了',reasonCode:'line-fault'})
  ...
  expect(state).toMatchObject({phase:'fault',faultReason:'双格要双脚落地',reasonCode:'double-feet-required'})
})
