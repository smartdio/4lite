...
test('seven-point scoring alternates serve and requires a two-point lead',async({page})=>{
  ...
  state=await page.evaluate(()=>window.__CAMPUS_TEST__.awardPingPongPoint('player','test-award'))
  expect(state).toMatchObject({feedbackCode:'point-player',reasonCode:'test-award'})
  ...
  state=await page.evaluate(()=>window.__CAMPUS_TEST__.awardPingPongPoint('ai','test-award'))
  expect(state).toMatchObject({feedbackCode:'point-computer',reasonCode:'test-award'})
  state=await page.evaluate(()=>window.__CAMPUS_TEST__.advancePingPong(1))
  ...
  state=await page.evaluate(()=>window.__CAMPUS_TEST__.awardPingPongPoint('player','test-award'))
  expect(state).toMatchObject({phase:'matchEnd',scores:{player:8,ai:6},feedbackCode:'match-won',reasonCode:'test-award'})
})
...
test('ping-pong score and point feedback use the shared arcade-comic WebGL HUD',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>window.__CAMPUS_TEST__.hud().pingPong)).toMatchObject({
    loaded:true,visible:true,mode:'match',playerScore:0,aiScore:0,server:'player',prompt:'点击抛球',
  ...
  await page.evaluate(()=>window.__CAMPUS_TEST__.awardPingPongPoint('player','test-award'))
  ...
  await page.evaluate(()=>window.__CAMPUS_TEST__.awardPingPongPoint('ai','test-award'))
